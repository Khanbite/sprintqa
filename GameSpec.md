# Game Design Document

## Game Title
**[Your game name]**

## Game Type
**[Dungeon Crawler / Text Adventure / Card Game / Trivia / Other]**

---

## Win Condition
**[What does the player do to win? Be specific.]**

---

## Lose Condition
**[What causes a loss? Can they tie?]**

---

## Core Mechanics
**(3–5 features that MUST work)**

1. **[e.g., Move between rooms]**
2. **[e.g., Health decreases when damaged]**
3. **[e.g., Combat system]**
4. *[Optional]*
5. *[Optional]*

---

## Player Input
**(What does the user type?)**

- **[e.g., Commands: move north/south/east/west, attack, inventory, quit]**
- **[e.g., Enter a number between 1 and 100]**

---

## Inputs We Will Validate
**(QA Test Ideas)**

- **[What if user enters "xyz"?]**
- **[What if they enter -5?]**
- **[What if they enter 999?]**

---

## Known Limitations
**(Be honest)**

- **[What will we NOT have time to build? Fancy graphics? AI opponents? Multiplayer?]**

---

## Tech Stack

- **[Python in terminal]**  
  **OR**
- **[Replit web game]**  
  **OR**
- **[Local Python setup]**
